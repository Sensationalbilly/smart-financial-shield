...
app.post("/api/signup", (req, res) => {
  ...
  if (!userIdentifier || !password) return res.status(400).json({ error: "email and password required" });

  const users = readJson(USERS_FILE);
  if (users.find(u => u.email === userIdentifier || u.username === userIdentifier)) {
  ...
  const newUser = { 
    id: userId, 
    firstname: firstname || userIdentifier.split('@')[0],
    lastname: lastname || '',
  ...
  res.json({ message: "ok", userId, token });
});
...
app.post("/api/login", (req, res) => {
  ...
  const userIdentifier = email || username;
  const users = readJson(USERS_FILE);
  const user = users.find(u => (u.email === userIdentifier || u.username === userIdentifier) && u.password === password);
...
app.get('/', (req, res) => {
  res.send('<h1>Smart Financial Shield Backend is running!</h1>');
});
